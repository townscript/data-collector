...
import {RecordSearchData} from "./record-search-data.service";
...
export class SearchDataCollector {
    ...
    static getSearchStreamData = (absoluteUrl:string, relativeUrl:string,
        ...
        let userAgentBrowser: string = SearchDataCollector.getWebBrowserInfo();
        let recordId: string = UUID.generateUUID32();
        let userDevice: string = SearchDataCollector.isDevicePhone() ? "Phone" : "Desktop";
    ...
    static getWebBrowserInfo = ():string => {
      try{
          ...
          M= ua.match(/(CriOS|opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i) || [];
